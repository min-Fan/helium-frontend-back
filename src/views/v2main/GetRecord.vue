<template>
  <div class="GetRecord" :class="hock ? 'GetRecord_hock' : ''">
    <div class="record_top">
      <van-nav-bar fixed :placeholder="true" :title="i18n.GetRecord.Record" left-arrow safe-area-inset-top
        @click-left="back">
        <template #right>
          <img v-show="!hock" class="filter_icon" src="../../assets/v2img/filter_icon.png" alt=""
            @click="isShow = true">
          <img v-show="hock" class="filter_icon" src="../../assets/v2img/filter_icon_hock.png" alt=""
            @click="isShow = true">
        </template>
      </van-nav-bar>
    </div>
    <div class="record_box">
      ...
      <div class="record_item clearfix" v-for="(item, index) in record" :key="index">
        ...
        <div class="record_usdt">
          <span>{{ Number(item.reward_amount).toFixed(4) }} USDT</span>
...
import { GetRecordGetReward, GetNewRecordGetReward } from '@api'
...
export default {
  ...
  methods: {
    ...
    async getRecord(limit = 0) {
      console.log(this.$route.query.parentPage)
      let params = {
        addr: this.address,
        desc: true,
        limit,
      ...
      try {
        ...
        if (this.$route.query.parentPage && this.$route.query.parentPage == 'HashRecord') {
          record = await GetNewRecordGetReward({ ...params })
        } else {
          record = await GetRecordGetReward({ ...params })
        }
        console.log(record)
        ...
        Promise.all(data.map(v => {
          ...
          obj.reward_amount = this.vuex_web3.utils.fromWei(parseInt(BigNumber(v.reward_amount)).toString(), 'ether')
          arr.push(obj)
        }))
        this.record = arr
