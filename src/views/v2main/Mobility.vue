<template>
  <div class="Mobility" :class="hock ? 'Mobility_hock' : ''">
    <div class="top">
    ...
    <div class="Mobility_tab">
      <div class="tab_exchange" v-show="active == '2'">
        <div class="exchange_inp clearfix">
          <div class="balance fl clearfix">
            <!-- <span class="fl">{{i18n.Trading.Send}}</span> -->
            <div @click="allBalance">
              <span class="fl" v-show="isExchange">{{ i18n.Trading.Balance }}：</span>
              <span class="fl" v-show="isExchange">{{ USDTbalance }}</span>
              <span class="fl" v-show="!isExchange">{{ i18n.Trading.Balance }}：</span>
              <span class="fl" v-show="!isExchange">{{ lpbalance }}</span>
            </div>
            <span class="fr tiggle" @click="changeToggle">
              {{ i18n.Mobility.Toggle }}
            </span>
          </div>
          <div class="inp_box_out fl">
            <van-field class="usdt_inp" v-show="isExchange" v-model="value1" :placeholder="i18n.Trading.OutNums"
              @blur="usdtInp" />
            <van-field class="esg_inp" v-show="!isExchange" v-model="value1" :placeholder="i18n.Trading.OutNums"
              :readonly="!isExchange" />
          ...
          <div class="change_status">
            <div class="change_box" v-show="!lpType" @click="changeStatus">
              <img v-show="!hock" src="../../assets/v2img/shangxia_icon.png" alt="">
              <img v-show="hock" src="../../assets/v2img/shangxia_icon_hock.png" alt="">
            </div>
            <div class="lp_exchange" v-show="lpType">
              <img src="../../assets/v2img/xia_dan.png" alt="">
            </div>
          </div>
          <div :class="lpType ? 'in_top_box' : ''" class="in_box fl">
            <div class="in_title clearfix" v-show="!lpType">
              <span class="fl" v-show="isExchange">{{ i18n.Trading.Balance }}：</span>
              <span class="fl" v-show="isExchange">{{ ESGbalance }}</span>
              <span class="fl" v-show="!isExchange">{{ i18n.Trading.Balance }}：</span>
              <span class="fl" v-show="!isExchange">{{ USDTbalance }}</span>
            </div>
            <div class="lp_exchange1" v-show="lpType">
              <div class="get_nums">
                <span class="lp_nums">{{ Number(usdtLp).toFixed(4) }}</span>
                <span class="lp_uint">USDT</span>
              </div>
              <van-icon name="plus" />
              <div class="get_nums">
                <span class="lp_nums">{{ Number(esgLp).toFixed(4) }}</span>
                <span class="lp_uint">ESG</span>
              </div>
            </div>
            <div class="inp_box_in" v-show="!lpType">
              ...
              <div class="lp_exchange1" v-show="!isExchange">
                <div class="get_nums">
                  <span class="lp_nums">{{ Number(usdtLp).toFixed(4) }}</span>
                  <span class="lp_uint">USDT</span>
                </div>
              ...
              <van-field class="esg_inp" v-show="isExchange" v-model="value2"
                :placeholder="isExchange ? i18n.Trading.OutNums : i18n.Trading.InNums" @blur="esgInp" />
              <!-- <div class="usdt_icon" v-show="!isExchange">
                                <img src="../../assets/v2img/usdt_icon.png" alt="" class="fl">
                                <span class="fl">USDT</span>
                            </div> -->
          ...
          <div class="exchange_btn fl">
            <div class="confirm_btn" v-show="isExchange" @click="openInvite">{{ i18n.Mobility.AddMobility }}</div>
            <div class="confirm_btn" v-show="!isExchange" @click="openInvite">{{ i18n.Mobility.RemogeMobility }}</div>
          </div>
          <div class="key_bnb fl">
            <!-- <div class="key clearfix">
                            <div class="key_left fl">
                                <span>KLP {{i18n.Trading.Value}}</span>
                            </div>
                            <div class="key_right fr">
                                <span>{{KEYbalance}} KLP</span>
                            </div>
                        </div> -->
            <div class="key clearfix">
              <div class="key_left fl">
              ...
              <div class="key_right fr">
                <span>{{ gasFee == 0 ? 0 : Number(gasFee).toFixed(4) }} ESG</span>
    ...
    <hold v-show="isShowHold" :isShowHold="isShowHold" :holdList="actions" @chooseItem="setChooseItem"
      @closeHode="closeHode" :type="3"></hold>
  </div>
</template>
...
// 导入以太坊合约配置文件
import { Eth } from '@eth'
import { contractMethods, GetEsgPrice, GetLpUserInfos, GetHistoryAddLpEsg } from '@api'
...
export default {
  ...
  methods: {
    ...
    toMove() {
      ...
      this.$router.push({ name: 'ExchangeRecord', query: { type: 3 } })
    },
    closeHode() {
    ...
    getLpPrice(type = 1) {
      if (type == 1) {
        let val = this.vuex_web3.utils.toWei(this.value1, 'ether')
        this.vuex_myContract.contrart_AndUandStaker.methods.GetUsdtAndEsgAmount(val).call().then(res => {
          this.usdtLp = this.vuex_web3.utils.fromWei(res[0], 'ether')
          this.esgLp = this.vuex_web3.utils.fromWei(res[1], 'ether')
          this.gasFee = this.esgLp * 0.05
        })
      } else if (type == 2) {
        let val = this.vuex_web3.utils.toWei(this.value1, 'ether')
        this.vuex_myContract.contrart_AndUandStaker.methods.GetRemoveLpForUsdtAndEsg(val).call().then(res => {
          this.usdtLp = this.vuex_web3.utils.fromWei(res[0], 'ether')
          this.esgLp = this.vuex_web3.utils.fromWei(res[1], 'ether')
          this.gasFee = this.esgLp * 0.05
        })
    ...
    setChooseItem(item) {
      ...
      this.value1 = Number(item.lp_amount).toFixed(4)
      this.gasFee = item.esg_amount * 0.05
      this.getLpPrice(2)
    ...
    // 获取余额 - klp
    getBalanceKey() {
      this.vuex_myContract.contrart_Klp.methods.balanceOf(this.address).call().then(res => {
        if (res != 0) {
          this.$nextTick(() => {
    ...
    // 获取用户lp信息
    getLpInfo() {
      GetLpUserInfos({ addr: this.address }).then(res => {
        if (res.code != 7) {
          let data = res.data
          this.$nextTick(() => {
            this.lpbalance = Number(this.vuex_web3.utils.fromWei(data.personal_total_lp, 'ether')).toFixed(4)
          })
        }
      })
    },
    ...
    async approved() {
      return new Promise(async (resolve, reject) => {
        ...
        await this.vuex_myContract.contrart_USDT.methods.allowance(this.address, Eth.contract.contrart_AndUandStaker.addr).call().then(res => {
          console.log("USDT approve ==>", res)
        ...
        let data = {
          ...
          arguments: [
            Eth.contract.contrart_AndUandStaker.addr,
    ...
    async approvedESG() {
      return new Promise(async (resolve, reject) => {
        ...
        // 查询授权
        let isApproved = false
        await this.vuex_myContract.contrart_ESG.methods.allowance(this.address, Eth.contract.contrart_AndUandStaker.addr).call().then(res => {
          console.log("ESG approve ==>", res)
          let data = _float.div(res, Math.pow(10, 18))
          if (data >= this.value1) {
            isApproved = true
          } else {
    ...
    async approvedKey() {
      return new Promise(async (resolve, reject) => {
        ...
        // 查询授权
        let isApproved = false
        await this.vuex_myContract.contrart_Klp.methods.allowance(this.address, Eth.contract.contrart_AndUandStaker.addr).call().then(res => {
          console.log("Klp approve ==>", res)
          let data = _float.div(res, Math.pow(10, 18))
          if (data >= this.value1) {
            isApproved = true
          } else {
        ...
        // 发起授权
        await contractMethods(this.address, Eth.contract.contrart_Klp.addr, data).then(async res => {
          if (res.code == 101) {
            Toast({ message: this.i18n.Toast.ApprovalSuccess, icon: 'passed', className: 'successToast' });
            resolve()
          }
    ...
    async getReferrer(accounts) {
      // 查询推荐人
      let addr = ''
      await this.vuex_myContract.contrart_AndUandStaker.methods.RecommendOf(accounts).call().then(res => {
    ...
    openInvite() {
      ...
      } else if (this.isExchange && !this.lpType) {
    ...
    async lpExchange() {
      // 停止参与提示
      if (this.isExchange) {
        // Toast({ duration: 2000, message: "KLP" + this.i18n.BlindBox.ruleBalanceNot, icon: 'close', className: 'fail' })
        ...
        if (!this.lpType && Number(this.ESGbalance) < Number(this.value2)) {
          Toast({ duration: 2000, message: "ESG" + this.i18n.BlindBox.ruleBalanceNot, icon: 'close', className: 'fail' })
          return
        }
        ...
        let exchangeType = this.lpType ? 1 : 2
        console.log(exchangeType, this.inviteAddr)
        // return
        let params = {
          ...
          arguments: [usdtNum, exchangeType, this.inviteAddr], // 参数
        }

        await contractMethods(this.address, Eth.contract.contrart_AndUandStaker.addr, params, true, 2).then(async res => {
          if (res.code == 101) {
            Toast.clear();
            Toast({ message: this.i18n.Mobility.AddSuc, icon: 'passed', className: 'successToast' });
            this.value1 = ''
            this.value2 = ''
            this.showInp = false
            setTimeout(() => {
        ...
        let params = {
          ...
          arguments: [idx, isTeam], // 参数
        }

        await contractMethods(this.address, Eth.contract.contrart_AndUandStaker.addr, params, true, 2).then(async res => {
          if (res.code == 101) {
            Toast.clear();
            Toast({ message: this.i18n.Mobility.RemoveSuc, icon: 'passed', className: 'successToast' });
            this.value1 = ''
            this.value2 = ''
            this.usdtLp = ''
            this.esgLp = ''
    ...
    .tab_exchange {
      ...
      .exchange_inp {
        ...
        .balance {
          ...
          .tiggle {
            color: #13C087;
            border-bottom: 2px solid #13C087;
          }
        }
...
.Mobility_hock {
  ...
  .Mobility_tab .tab_exchange .exchange_inp {
    background-color: $back-hock;
    box-shadow: none;
  }

  .Mobility_tab .tab_exchange .exchange_inp .in_box .lp_exchange1 {
    background-color: $back-qs-hock;
  }

  .Mobility_tab .tab_exchange .exchange_inp .in_box .lp_exchange1 .lp_nums {
    color: $text-hock;
  }

  .Mobility_tab .tab_exchange .exchange_record {
    background-color: $back-hock;
    box-shadow: none;
  }

  .Mobility_tab .tab_exchange .exchange_inp .change_status .change_box {
    background-color: $trading-changebg-hock;
  }

  .Mobility_tab .tab_exchange .exchange_inp .balance span {
    color: $text-999-hock;
  }

  .Mobility_tab .tab_exchange .exchange_inp .balance span:nth-child(2) {
    color: $text-qs-hock;
  }

  .Mobility_tab .tab_exchange .exchange_inp .inp_box_out,
  .Mobility_tab .tab_exchange .exchange_inp .inp_box_in {
    background-color: $back-qs-hock;
  }

  .Mobility_tab .tab_exchange .exchange_inp .in_box .in_title span:nth-of-type(2) {
    color: $text-qs-hock;
  }
  ...
  .Mobility_tab .tab_exchange .exchange_inp .inp_box_in /deep/ .van-cell input,
  .Mobility_tab .tab_exchange .exchange_inp .inp_box_out /deep/ .van-cell input {
    color: $text-hock;
  ...
  .Mobility_tab .tab_exchange .exchange_inp .inp_box_out .usdt_icon,
  .Mobility_tab .tab_exchange .exchange_inp .inp_box_in .esg_icon {
    background-color: $back-hock;
  ...
  .Mobility_tab .tab_exchange .exchange_inp .key_bnb {
    border: 2px solid $trading-border-hock;
    border-radius: 24px;
  }

  .Mobility_tab .tab_exchange .exchange_record .record_item .record_detail .record_icon::after {
    background-color: $trading-border-hock;
  }

  .Mobility_tab .tab_exchange .exchange_inp .inp_box_out .esg_icon,
  .Mobility_tab .tab_exchange .exchange_inp .inp_box_in .usdt_icon {
    background-color: $back-hock;
  }

  .Mobility_tab .tab_exchange .exchange_inp .inp_box_out .esg_icon span,
  .Mobility_tab .tab_exchange .exchange_inp .inp_box_in .usdt_icon span {
    color: $text-qs-hock !important;
  }
  ...
  .Mobility_tab .tab_exchange .exchange_inp .in_box .in_title {
    & span:nth-of-type(2) {
      color: $text-hock;
    }
  ...
  /deep/ .Mobility_tab .tab_exchange .exchange_record .record_item .van-collapse-item__wrapper .van-collapse-item__content {
    border-color: $bpos-border-b-hock;
  }

  /deep/ .Mobility_tab .tab_exchange .exchange_record .record_item .van-collapse-item__wrapper .van-collapse-item__content .record_ontent .content_item {
    color: $text-qqs-hock;
  }
}
</style>
