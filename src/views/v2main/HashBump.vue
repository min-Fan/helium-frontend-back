<template>
  <div class="Hash" :class="hock ? 'Hash_hock' : ''">
    <div class="top">
    ...
    <div class="Hash_tab">
      <div class="tab_exchange" v-show="active == '5'">
        <div class="exchange_inp clearfix">
          <div class="balance fl clearfix">
            ...
            <span class="fr">
              ...
              <div v-show="!isExchange">{{ i18n.Trading.Balance }}：{{ holdBalance == 0 ? 0 :
                  Number(holdBalance).toFixed(4)
              }}</div>
          ...
          <div class="in_box fl">
            <div class="in_title clearfix">
              ...
              <span class="fr" v-show="!isExchange" @click="allBalance">{{ i18n.Trading.Balance }}：{{ USDTbalance
              }}</span>
              <span class="fr" v-show="isExchange">{{ i18n.Trading.Balance }}：{{ holdBalance == 0 ? 0 :
                  Number(holdBalance).toFixed(4)
              }}</span>
            </div>
            <div class="inp_box_in">
              <van-field class="usdt_inp" v-show="!isExchange" v-model="value2" :placeholder="i18n.Trading.InNums"
                @blur="usdtInp" :readonly="!isExchange" />
              <van-field class="esg_inp" v-show="isExchange" v-model="value2" :placeholder="i18n.Trading.InNums"
                @blur="esgInp" />
              <div class="usdt_icon" v-show="!isExchange">
                <img src="../../assets/v2img/usdt_icon.png" alt="" class="fl">
                <span class="fl">USDT</span>
      ...
      <div class="game_detail">
        <van-collapse v-model="activeGamse">
          <van-collapse-item :title="i18n.HashBump.GameDetail" name="1">
            <span class="game_html" v-html="i18n.HashBump.Gamehtml"></span>
            <div class="toggle_icon" @click="upClick">
              <van-icon name="arrow-up" />
              <span>{{ i18n.HashBump.Uptext }}</span>
            </div>
          </van-collapse-item>
        </van-collapse>
      </div>

      <div class="hash_model">
        <HashModel></HashModel>
      </div>
    </div>
...
import HashModel from './HashModel.vue'
// 导入以太坊合约配置文件
import { Eth } from '@eth'
import { contractMethods, HandlingFees, GetNewHistoryBuyEsg, GetEsgPrice, GetNewBposUserInfos } from '@api'
...
export default {
  ...
  methods: {
    ...
    // 选择持仓
    setChooseItem(item) {
      if (Number(item.reward) < Number(item.usdt_amount)) {
        Toast({ duration: 2000, message: this.i18n.HashBump.Lockout, icon: 'close', className: 'fail' });
        return
      }
    ...
    // 获取余额 - hash
    getBalanceHash() {
      this.vuex_myContract.contrart_Hash.methods.balanceOf(this.address).call().then(res => {
        console.log(res)
        if (res != 0) {
          this.HASHbalance = Number(this.vuex_web3.utils.fromWei(res, 'ether')).toFixed(4)
        }
      })
    },
    // 获取汇率
    ...
    async getHoldList() {
      ...
      data.map(v => {
        ...
        obj.reward = this.vuex_web3.utils.fromWei(v.reward, 'ether')
    ...
    async approved() {
      return new Promise(async (resolve, reject) => {
        ...
        await this.vuex_myContract.contrart_USDT.methods.allowance(this.address, Eth.contract.contrart_NewBuyStaker.addr).call().then(res => {
        ...
        let data = {
          ...
          arguments: [
            Eth.contract.contrart_NewBuyStaker.addr,
    ...
    async approvedHash() {
      return new Promise(async (resolve, reject) => {
        ...
        // 查询授权
        let isApproved = false
        await this.vuex_myContract.contrart_Hash.methods.allowance(this.address, Eth.contract.contrart_NewBuyStaker.addr).call().then(res => {
        ...
        // 发起授权
        await contractMethods(this.address, Eth.contract.contrart_Hash.addr, data).then(async res => {
          if (res.code == 101) {
            Toast({ message: this.i18n.Toast.ApprovalSuccess, icon: 'passed', className: 'successToast' });
            resolve()
          }
    ...
    async getReferrer(accounts) {
      // 查询推荐人
      let addr = ''
      await this.vuex_myContract.contrart_NewBuyStaker.methods.RecommendOf(accounts).call().then(res => {
    ...
    async gasPriceAmount() {
      if (this.isExchange) {
        ...
        if (this.HASHbalance == 0) return

        if (Number(this.HASHbalance) < Number(this.value1)) return
        // 参数
        ...
        console.log(params)
        HandlingFees(this.address, Eth.contract.contrart_NewBuyStaker.addr, params).then(async res => {
          if (res.code == 101) {
    ...
    async Exchange() {
      ...
      if (this.isExchange) {
        ...
        // if (this.HASHbalance == 0) {
        //   Toast({ duration: 2000, message: "HASH" + this.i18n.BlindBox.ruleBalanceNot, icon: 'close', className: 'fail' })
        //   return
        // }

        // if (Number(this.HASHbalance) < Number(this.value1)) {
        //   Toast({ duration: 2000, message: "HASH" + this.i18n.BlindBox.ruleBalanceNot, icon: 'close', className: 'fail' })
        //   return
        // }
        ...
        // 验证授权 Hash
        // await this.approvedHash().then(async (res) => {
        //   setTimeout(async () => {
            ...
            await contractMethods(this.address, Eth.contract.contrart_NewBuyStaker.addr, params, true, 5).then(async res => {
              if (res.code == 101) {
                Toast.clear();
        ...
        //   }, 2000);
        // }).catch(err => {
        //   Toast({ duration: 2000, message: "Hash" + this.i18n.Toast.ApprovalFail, icon: 'close', className: 'fail' });
        //   Toast.clear();
        // })
...
.Hash_hock {
  ...
  .Hash_tab .tab_exchange .exchange_inp {
    background-color: $back-hock;
    box-shadow: none;
  }

  .Hash_tab .tab_exchange .exchange_record {
    background-color: $back-hock;
    box-shadow: none;
  }

  .Hash_tab .tab_exchange .exchange_inp .change_status .change_box {
    background-color: $trading-changebg-hock;
  }

  .Hash_tab .tab_exchange .exchange_inp .balance span {
    color: $text-999-hock;
  }

  .Hash_tab .tab_exchange .exchange_inp .balance span:nth-child(2) {
    color: $text-qs-hock;
  }

  .Hash_tab .tab_exchange .exchange_inp .inp_box_out,
  .Hash_tab .tab_exchange .exchange_inp .inp_box_in {
    background-color: $back-qs-hock;
  }

  .Hash_tab .tab_exchange .exchange_inp .in_box .in_title span:nth-of-type(2) {
    color: $text-qs-hock;
  }
  ...
  .Hash_tab .tab_exchange .exchange_inp .inp_box_in /deep/ .van-cell input,
  .Hash_tab .tab_exchange .exchange_inp .inp_box_out /deep/ .van-cell input {
    color: $text-hock;
  ...
  .Hash_tab .tab_exchange .exchange_inp .inp_box_out .usdt_icon,
  .Hash_tab .tab_exchange .exchange_inp .inp_box_in .esg_icon {
    background-color: $back-hock;
  ...
  .Hash_tab .tab_exchange .exchange_inp .key_bnb {
    border: 2px solid $trading-border-hock;
    border-radius: 24px;
  }

  .Hash_tab .tab_exchange .exchange_record .record_item .record_detail .record_icon::after {
    background-color: $trading-border-hock;
  }

  .Hash_tab .tab_exchange .exchange_inp .inp_box_out .esg_icon,
  .Hash_tab .tab_exchange .exchange_inp .inp_box_in .usdt_icon {
    background-color: $back-hock;
  }

  .Hash_tab .tab_exchange .exchange_inp .inp_box_out .esg_icon span,
  .Hash_tab .tab_exchange .exchange_inp .inp_box_in .usdt_icon span {
    color: $text-qs-hock !important;
  }
  ...
  .Hash_tab .tab_exchange .exchange_inp .in_box .in_title {
    & span:nth-of-type(2) {
      color: $text-hock;
    }
  ...
  /deep/ .Hash_tab .tab_exchange .exchange_record .record_item .van-collapse-item__wrapper .van-collapse-item__content {
    border-color: $bpos-border-b-hock;
  }

  /deep/ .Hash_tab .tab_exchange .exchange_record .record_item .van-collapse-item__wrapper .van-collapse-item__content .record_ontent .content_item {
    color: $text-qqs-hock;
  }

  .Hash_tab .game_detail {
    box-shadow: none;
  }

  /deep/ .Hash_tab .game_detail .van-collapse .van-collapse-item__content {
    background-color: #09130F;
  }

  /deep/ .Hash_tab .game_detail .van-collapse .van-collapse-item__content .game_html {
    color: #fff;
  }

  /deep/ .Hash_tab .game_detail .van-collapse .van-collapse-item__title--expanded::after {
    border: 2px solid #28342E;
    margin-bottom: 20px;
  }
