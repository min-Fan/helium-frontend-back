<template>
    <div class="ShareDetails" :class="hock ? 'ShareDetails_hock':''">
        <div class="details_top">
            <van-nav-bar fixed :placeholder="true" :title="i18n.LpDetails.FeneDetails" left-arrow safe-area-inset-top @click-left="back"></van-nav-bar>
        </div>
        <div></div>
        <div class="tab_hold">
            <div class="hold_total">
                <div class="text_record clearfix">
                    <span class="fl">{{i18n.LpDetails.FenePrice}}（USDT）</span>
                </div>
                <div class="total clearfix">
                    ...
                    <div class="team_daishi clearfix" v-if="isTeam">
                        ...
                        <span class="fl">{{Number(NoRelease).toFixed(4)}} ESG</span>
                    </div>
                </div>
                <div class="total_get clearfix">
                    <div class="total_nums fl clearfix">
                        <p>{{i18n.LpDetails.FenNums}}(ESG/USDT)</p>
                        <p>{{Number(info.esg_amount).toFixed(4)}}</p>
                        <p>{{Number(info.usdt_amount).toFixed(4)}}</p>
                        <!-- <span class="fl">239.1235</span> -->
                    </div>
                    <div class="revenue fl">
                        <p>{{i18n.ShareDetails.NoAndYes}}(ESG)</p>
                        <p>{{Number(info.norecieved).toFixed(4)}}</p>
                        <!-- <i></i> -->
                        <p class="money">{{Number(info.reward).toFixed(4)}}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="record_item" v-for="(item, index) in list" :key="index">
                        <div class="item_team" v-if="item.type == 'order' && isTeam">
                            ...
                            <div class="node_detail clearfix">
                                ...
                                <span class="fr money">+{{Number(item.reward_amount).toFixed(4)}} ESG</span>
                            </div>
                        </div>
        ...
        <div class="get_flxedbtn_box">
            <div class="get_flxedbtn">
                <div class="esg_money">
                    ...
                    <span>{{Number(info.norecieved).toFixed(4)}} ESG</span>
                </div>
...
// 导入以太坊合约配置文件
import { Eth } from '@eth'
import { contractMethods, GetLpNotReceived, GetHistoryAddLpEsg } from '@api'
...
export default {
    ...
    methods: {
        ...
        async getTotalInfo() {
            ...
            let res = await GetHistoryAddLpEsg({...params})
            let data = res.data
            ...
            this.total = Number(this.info.esg_amount * this.priceRatio) + Number(this.info.usdt_amount)
        ...
        async getRecord(status = 'weiling', limit = 0) {
            ...
            try {
                let res = await GetLpNotReceived({...params})
                let data = res.data
        ...
        async getEarnings() {
            ...
            console.log("this.info.chain_record_idx ==>", this.info.chain_record_idx)
